...
# You can ofset the video from the audio if necessary is in seconds
Offset=0.0

libcamera-vid -n -t 0 --width $Width --height $Height --hdr $HDR --inline --framerate 25 -b $Bitrate -o - | \
ffmpeg -re -i - \
-re -f alsa -ac 1 -itsoffset $Offset -thread_queue_size 12000 -i hw:$InputA \
-map 0:v:0 -map 1:a:0 -map_metadata:g 1:g \
...
# - Add Video
# - Add audio
# - Set whats audio and what video
# - transcode Video
# - Trancode Audio
# - Send to RTPM Server
